...
- [x] #160 (upstream desktop#34) — FileCloud-style quick share: Explorer "Share link…" submenu item opens a share dialog window (new `ManagerCommand::ShareLink` → `OpenShareDialog` event → dedicated `share` webview window + `/share` route) with permission mode (view+download / preview-only / edit / folder upload-only), expiry, password, and download limit mapped to the full `PUT /share` surface (`ShareCreateService` gained the option fields); "Copy share link" stays as the one-click public-link action alongside it; on Linux/macOS the same dialog is reachable via the `cloudreve://share/<path>` deep link — `desktop/linux/` ships a `cloudreve-share` helper plus Nautilus script and Dolphin service-menu definitions
- [x] In-app updates — desktop: `tauri-plugin-updater` checks `releases/latest/latest.json` at startup (prompted-version persisted to avoid re-nagging), `/update` window shows release notes + live download overlay + relaunch, manual check in About; minisign keypair signs updater artifacts, `updater-manifest` job publishes `latest.json` in both release workflows; Android: `app-update-ktx` IMMEDIATE flow — resume-time check → once-per-version dialog → official Play update sheet
- [x] Full locale coverage for ported features — ~440 previously-missing keys per locale filled across `application`/`dashboard`/`common` in all 10 non-en/zh locales (SMS auth, SSO errors, ACLs, paid shares, vault, CDN routes, relocation, gift codes, delegated admin, abuse reports); zh-TW via OpenCC `s2twp`; placeholder/Trans-tag fidelity audit fixed `{{eta}}`/`{{time}}`/`{{message}}` drops and malformed tags; Android update dialog localized via `values-*/strings.xml` in 12 locales
