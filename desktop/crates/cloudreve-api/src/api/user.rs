...
pub trait UserApi {
    ...
    async fn get_credit_log(
        &self,
        params: &GetCreditLogService,
    ) -> ApiResult<CreditChangeLogResponse>;
...
impl UserApi for Client {
    ...
    async fn patch_user_settings(&self, settings: &PatchUserSetting) -> ApiResult<()> {
        self.patch("/user/setting", settings, RequestOptions::new())
            .await
    }
    ...
    async fn get_credit_log(
        ...
        self.get(&format!("/user/credit/log{}", query), RequestOptions::new())
            .await
    }
