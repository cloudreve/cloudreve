...
pub trait WorkflowApi {
    ...
    async fn set_download_files(
        &self,
        task_id: &str,
        request: &SetDownloadFilesService,
    ) -> ApiResult<()>;
}
...
impl WorkflowApi for Client {
    ...
    async fn import(&self, request: &ImportWorkflowService) -> ApiResult<TaskResponse> {
        self.post("/task/import", request, RequestOptions::new())
            .await
    }
    ...
    async fn list_tasks(&self, params: &ListTaskService) -> ApiResult<TaskListResponse> {
        let mut query_params = vec![
            ...
            format!(
                "category={}",
                serde_json::to_string(&params.category)
                    .unwrap_or_default()
                    .trim_matches('"')
            ),
        ];
        ...
        self.get(&format!("/task{}", query), RequestOptions::new())
            .await
    }

    async fn get_task(&self, task_id: &str) -> ApiResult<TaskResponse> {
        self.get(&format!("/task/{}", task_id), RequestOptions::new())
            .await
    }
    ...
    async fn delete_task(&self, task_id: &str) -> ApiResult<()> {
        self.delete(&format!("/task/{}", task_id), RequestOptions::new())
            .await
    }
