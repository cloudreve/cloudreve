...
const (
	...
	// EdgeStoragePoliciesAllowed holds the string denoting the storage_policies_allowed edge name in mutations.
	EdgeStoragePoliciesAllowed = "storage_policies_allowed"
	...
	// StoragePoliciesAllowedTable is the table that holds the storage_policies_allowed relation/edge. The primary key declared below.
	StoragePoliciesAllowedTable = "group_storage_policies_allowed"
	// StoragePoliciesAllowedInverseTable is the table name for the StoragePolicy entity.
	// It exists in this package in order to avoid circular dependency with the "storagepolicy" package.
	StoragePoliciesAllowedInverseTable = "storage_policies"
)
...
var (
	// StoragePoliciesAllowedPrimaryKey and StoragePoliciesAllowedColumn2 are the table columns denoting the
	// primary key for the storage_policies_allowed relation (M2M).
	StoragePoliciesAllowedPrimaryKey = []string{"group_id", "storage_policy_id"}
)
...
// ByStoragePoliciesAllowedCount orders the results by storage_policies_allowed count.
func ByStoragePoliciesAllowedCount(opts ...sql.OrderTermOption) OrderOption {
	return func(s *sql.Selector) {
		sqlgraph.OrderByNeighborsCount(s, newStoragePoliciesAllowedStep(), opts...)
	}
}

// ByStoragePoliciesAllowed orders the results by storage_policies_allowed terms.
func ByStoragePoliciesAllowed(term sql.OrderTerm, terms ...sql.OrderTerm) OrderOption {
	return func(s *sql.Selector) {
		sqlgraph.OrderByNeighborTerms(s, newStoragePoliciesAllowedStep(), append([]sql.OrderTerm{term}, terms...)...)
	}
}
...
func newStoragePoliciesAllowedStep() *sqlgraph.Step {
	return sqlgraph.NewStep(
		sqlgraph.From(Table, FieldID),
		sqlgraph.To(StoragePoliciesAllowedInverseTable, FieldID),
		sqlgraph.Edge(sqlgraph.M2M, false, StoragePoliciesAllowedTable, StoragePoliciesAllowedPrimaryKey...),
	)
}
