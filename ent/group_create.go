...
func (gc *GroupCreate) AddStoragePoliciesAllowedIDs(ids ...int) *GroupCreate {
	gc.mutation.AddStoragePoliciesAllowedIDs(ids...)
	return gc
}

// AddStoragePoliciesAllowed adds the "storage_policies_allowed" edges to the StoragePolicy entity.
func (gc *GroupCreate) AddStoragePoliciesAllowed(s ...*StoragePolicy) *GroupCreate {
	ids := make([]int, len(s))
	for i := range s {
		ids[i] = s[i].ID
	}
	return gc.AddStoragePoliciesAllowedIDs(ids...)
}
...
func (gc *GroupCreate) createSpec() (*Group, *sqlgraph.CreateSpec) {
	...
	if nodes := gc.mutation.StoragePoliciesAllowedIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
