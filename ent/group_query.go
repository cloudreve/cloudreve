...
type GroupQuery struct {
	...
	withAllowedPolicies *StoragePolicyQuery
...
func (gq *GroupQuery) QueryMemberships() *GroupMembershipQuery {
...
// QueryAllowedPolicies chains the current query on the "allowed_policies" edge.
func (gq *GroupQuery) QueryAllowedPolicies() *StoragePolicyQuery {
	query := (&StoragePolicyClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withAllowedPolicies: gq.withAllowedPolicies.Clone(),
...
func (gq *GroupQuery) WithMemberships(opts ...func(*GroupMembershipQuery)) *GroupQuery {
	query := (&GroupMembershipClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithAllowedPolicies tells the query-builder to eager-load the nodes that are connected to
// the "allowed_policies" edge. The optional arguments are used to configure the query builder of the edge.
func (gq *GroupQuery) WithAllowedPolicies(opts ...func(*StoragePolicyQuery)) *GroupQuery {
	query := (&StoragePolicyClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withAllowedPolicies = query
	return gq
}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withAllowedPolicies; query != nil {
		if err := gq.loadAllowedPolicies(ctx, query, nodes,
			func(n *Group) { n.Edges.AllowedPolicies = []*StoragePolicy{} },
			func(n *Group, e *StoragePolicy) { n.Edges.AllowedPolicies = append(n.Edges.AllowedPolicies, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadMemberships(ctx context.Context, query *GroupMembershipQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupMembership)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (gq *GroupQuery) loadAllowedPolicies(ctx context.Context, query *StoragePolicyQuery, nodes []*Group, init func(*Group), assign func(*Group, *StoragePolicy)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.AllowedPoliciesTable)
		s.Join(joinT).On(s.C(storagepolicy.FieldID), joinT.C(group.AllowedPoliciesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(group.AllowedPoliciesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.AllowedPoliciesPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "allowed_policies" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
