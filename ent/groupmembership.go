...
func (*GroupMembership) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case groupmembership.FieldID, groupmembership.FieldUserID, groupmembership.FieldGroupID:
			values[i] = new(sql.NullInt64)
		case groupmembership.FieldCreatedAt, groupmembership.FieldUpdatedAt, groupmembership.FieldDeletedAt, groupmembership.FieldExpires:
	...
	for i := range columns {
		...
		case groupmembership.FieldExpires:
			if value, ok := values[i].(*sql.NullTime); !ok {
				...
				gm.Expires = new(time.Time)
				*gm.Expires = value.Time
			}
		default:
...
func (e *GroupMembership) SetUser(v *User) {
	e.Edges.User = v
	e.Edges.loadedTypes[0] = true
}

// SetGroup manually set the edge as loaded state.
func (e *GroupMembership) SetGroup(v *Group) {
	e.Edges.Group = v
