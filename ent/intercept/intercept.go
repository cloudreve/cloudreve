...
type AclEntryFunc func(context.Context, *ent.AclEntryQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f AclEntryFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.AclEntryQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.AclEntryQuery", q)
}

// The TraverseAclEntry type is an adapter to allow the use of ordinary function as Traverser.
type TraverseAclEntry func(context.Context, *ent.AclEntryQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseAclEntry) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseAclEntry) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.AclEntryQuery); ok {
		return f(ctx, q)
	}
...
type InvitationCodeFunc func(context.Context, *ent.InvitationCodeQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f InvitationCodeFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.InvitationCodeQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.InvitationCodeQuery", q)
}

// The TraverseInvitationCode type is an adapter to allow the use of ordinary function as Traverser.
type TraverseInvitationCode func(context.Context, *ent.InvitationCodeQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseInvitationCode) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseInvitationCode) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.InvitationCodeQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.InvitationCodeQuery", q)
}
