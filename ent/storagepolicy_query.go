...
// QueryGroupsAllowed chains the current query on the "groups_allowed" edge.
func (spq *StoragePolicyQuery) QueryGroupsAllowed() *GroupQuery {
	query := (&GroupClient{config: spq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := spq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(storagepolicy.Table, storagepolicy.FieldID, selector),
			sqlgraph.To(group.Table, group.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, storagepolicy.GroupsAllowedTable, storagepolicy.GroupsAllowedPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(spq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (spq *StoragePolicyQuery) Clone() *StoragePolicyQuery {
	...
	return &StoragePolicyQuery{
		...
		withGroupsAllowed: spq.withGroupsAllowed.Clone(),
...
// WithGroupsAllowed tells the query-builder to eager-load the nodes that are connected to
// the "groups_allowed" edge. The optional arguments are used to configure the query builder of the edge.
func (spq *StoragePolicyQuery) WithGroupsAllowed(opts ...func(*GroupQuery)) *StoragePolicyQuery {
	query := (&GroupClient{config: spq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	spq.withGroupsAllowed = query
	return spq
}
...
func (spq *StoragePolicyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*StoragePolicy, error) {
	...
	if query := spq.withGroupsAllowed; query != nil {
		if err := spq.loadGroupsAllowed(ctx, query, nodes,
			func(n *StoragePolicy) { n.Edges.GroupsAllowed = []*Group{} },
			func(n *StoragePolicy, e *Group) { n.Edges.GroupsAllowed = append(n.Edges.GroupsAllowed, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (spq *StoragePolicyQuery) loadGroupsAllowed(ctx context.Context, query *GroupQuery, nodes []*StoragePolicy, init func(*StoragePolicy), assign func(*StoragePolicy, *Group)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(storagepolicy.GroupsAllowedTable)
		s.Join(joinT).On(s.C(group.FieldID), joinT.C(storagepolicy.GroupsAllowedPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(storagepolicy.GroupsAllowedPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(storagepolicy.GroupsAllowedPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "groups_allowed" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
